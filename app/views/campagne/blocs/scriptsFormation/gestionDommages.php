<?php
/**
 * Script pour la gestion des dommages et changements de type sinistre
 * Gère l'affichage dynamique des dommages selon le type de sinistre sélectionné
 */
?>
<script>
// Gestion du changement de type de sinistre
function onChangeTypeSin() {
                ...
                if (typeSinistre.value == "brisGlace") {
                    ...
                    if (typeSinistre.value == "vandalisme") {
                        ...
                        if (typeSinistre.value == "evenementClimatique") {
                            $(`#textDommages`).text(
                                "Pouvez-vous me décrire les dégâts causés par l'événement climatique ?");
                            dommages = [];
                        } else {
    ...
    dommages.push("Autre");
    let html = "";
    const dommagesCoches = '<?= $questScript ? $questScript->dommages : '' ?>' // <- récupéré depuis PHP ou du formulaire
