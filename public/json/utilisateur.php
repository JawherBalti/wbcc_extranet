...
if (isset($_GET['action'])) {
    ...
    if ($action == "getUsersByCompany") {
        ...
        $connectedUserId = $_POST['connectedUserId'];

        $sql = "SELECT u.idUtilisateur, c.fullName
                  FROM wbcc_utilisateur u
                  LEFT JOIN wbcc_contact c ON u.idContactF = c.idContact
                  JOIN gestion_utilisateur_societe us ON u.idUtilisateur = us.utilisateur_id
                  JOIN gestion_societe_rym s ON us.societe_id = s.id
                  WHERE u.etatUser = 1 AND s.nom_societe = '$companyName'";
        if ($isAdmin == "false") {
            $sql .= " AND u.idUtilisateur = $connectedUserId";
        }
        $db->query($sql);
        $users = $db->resultset();
    ...
    if ($action == "getUsersBySite") {
        ...
        if ($isPointageInterne != "") {
            $req .= " AND isPointageInterne = $isPointageInterne";
        }

        // Fetch all users
        $query = "SELECT * FROM wbcc_contact a, wbcc_utilisateur b 
                  WHERE a.idContact = b.idContactF AND b.idSiteF = $idSite AND isInterne = 1 $req 
                  GROUP BY idUtilisateur 
                  ORDER BY a.prenomContact, a.nomContact ASC";

        $db->query($query);
        $users = $db->resultSet(); // Returns an array of objects

        // Get today's date
        $today = date('Y-m-d');

        // Fetch users on leave today
        $queryConge = "SELECT DISTINCT idUtilisateurF FROM wbcc_demandesconge 
                       WHERE dateDebutDeCongeReelle <= :today 
                       AND dateFinDeCongeReelle >= :today";
        $db->query($queryConge);
        $db->bind(':today', $today);
        $congeUsers = $db->resultSet(); // Returns an array of objects

        // Extract user IDs of those on leave
        $congeUserIds = array_map(function ($row) {
            return $row->idUtilisateurF; // Access property as object
        }, $congeUsers);

        // Filter out users on leave
        $filteredUsers = array_filter($users, function ($user) use ($congeUserIds) {
            return !in_array($user->idUtilisateur, $congeUserIds); // Access property as object
        });
