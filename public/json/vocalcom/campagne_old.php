...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == "liste") {
        $db->query("SELECT * FROM wbcc_campagne_vocalcom");
    ...
    if ($_GET['action'] == "recapJournalier" && isset($_GET['DID'], $_GET['StatusGroup'])) {
        ...
        //Appels émis du jour
        $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa  WHERE DID_CAMPAGNE = '$DID' AND DATE = '$yesterday' AND fa.STATUSGROUP = $StatusGroup");
        $appels = $db->single();
        ...
        foreach ($statusArg as $key => $stat) {
            ...
            $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa WHERE DID_CAMPAGNE = '$DID'  AND fa.STATUS = $statusCode AND fa.DETAIL = $statusDetail AND DATE = '$yesterday'");
            $res = $db->single();
            $data[] = [
    ...
    if ($_GET['action'] == "recapProduction" && isset($_GET['DID'], $_GET['StatusGroup'])) {
        ...
        $StatusGroup = $_GET['StatusGroup'];
        #DEBUT FA
        $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa WHERE DID_CAMPAGNE = '$DID' AND fa.STATUSGROUP = $StatusGroup");
        $appels = $db->single();
        //Qualifications
        $statusArg = getQualificationsArgByDID($DID);
        $statusPos = getQualificationsPosByDID($DID);
        ...
        foreach ($statusArg as $key => $stat) {
            $statusCode = $stat->StatusCode;
            $statusDetail = $stat->StatusDetail;
            $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa WHERE DID_CAMPAGNE = '$DID'  AND fa.STATUS = $statusCode AND fa.DETAIL = $statusDetail");
            $res = $db->single();
            $data[] = [
                "statutArg" => "Nombre Appels " . $stat->StatusText,
                "nbAppels" => my_formatNumberEspace($res->nbAppels)
    ...
    if ($_GET['action'] == "getEnregByQualif" && isset($_GET['DID'], $_GET['StatusCode'], $_GET['StatusDetail'])) {
        ...
        $db->query("SELECT * FROM wbcc_record_vocalcom as r, wbcc_fa_vocalcom as fa WHERE DID_CAMPAGNE = '$DID'  AND fa.STATUS = $statusCode AND fa.DETAIL = $statusDetail AND INDICE=indiceAppel ");
        $res = $db->resultSet();
    ...
    if ($_GET['action'] == 'save') {
        ...
        if ($camp) {
            $db->query("UPDATE wbcc_campagne_vocalcom SET Description=:description,Type=:type, State=:state,StatusGroup=:status, DBName=:dbname, DID=:DID WHERE Oid=:oid");
        } else {
            $db->query("INSERT INTO wbcc_campagne_vocalcom(Oid,Description,Type,State,DBName,DID,StatusGroup) VALUES (:oid, :description, :type, :state, :dbname, :DID, :status)");
        }
    ...
    if ($_GET['action'] == 'saveFA' && isset($_GET['DID'], $_GET['date'])) {
        ...
        $today = date("d-m-Y");
        $now = $_GET['date'];
        $db->query("DELETE FROM wbcc_fa_vocalcom WHERE DID_CAMPAGNE = '$DID' AND DATE='$now'");
        $db->execute();
        $ok = "ko";
        ...
        foreach ($_POST as $key => $appel) {
            ...
            if ($DATE == $now) {
                ...
                $db->query("INSERT INTO wbcc_fa_vocalcom(INDICE,DATE,HEURE,ID_TV,TV,STATUS,LIB_STATUS,HISTORIQUE,TEL,TEL1,NBAPPELS,DUREE,TEL_DISPLAY,DID_CAMPAGNE,STATUSGROUP,DETAIL,LIB_DETAIL) VALUES (:INDICE,:DATE,:HEURE,:ID_TV,:TV,:STATUS,:LIB_STATUS,:HISTORIQUE,:TEL,:TEL1,:NBAPPELS,:DUREE,:TEL_DISPLAY,:DID_CAMPAGNE,:STATUSGROUP,:DETAIL,:LIB_DETAIL)");
        ...
        echo json_encode("FA_$ok");
    }

    if ($_GET['action'] == 'saveAGENT') {
        $_POST = json_decode(file_get_contents('php://input'), true);

        $db->query("TRUNCATE wbcc_agent_vocalcom");
        $db->execute();
        $ok = "ko";
        foreach ($_POST as $key => $agent) {
            ...
            extract($identR);
            extract($humanR);
            $db->query("INSERT INTO wbcc_agent_vocalcom(Oid,FirstName,LastName,LoginName,Ident,Password,OutCampaigns,Email,WorkspaceOid) VALUES (:Oid,:FirstName,:LastName,:LoginName,:Ident,:Password,:OutCampaigns,:Email,:WorkspaceOid)");

            $db->bind("Oid", $Oid, null);
            $db->bind("FirstName", $FirstName, null);
            $db->bind("LastName", $LastName, null);
            $db->bind("LoginName", $LoginName, null);
            $db->bind("Ident", $Ident, null);
            ...
            if ($appel) {
                ...
                if (!file_exists("../../recordsVocalcom/" . $newName)) {
                    if (move_uploaded_file($filepath, "../../recordsVocalcom/" . $newName)) {
                        $db->query("INSERT INTO wbcc_record_vocalcom(DID,date,heure,indiceAppel,codeAgent,nomFichier,tailleFichier, infoAgent, qualification) 
                    ...
                    $db->query("UPDATE wbcc_record_vocalcom SET DID='$DID',date='$date',heure='$heure',indiceAppel='$indice',codeAgent='$codeAgent',nomFichier='$newName',tailleFichier='$size',infoAgent='$info', qualification='$statut' WHERE indiceAppel='$indice' AND DID='$DID' AND date='$date' AND heure='$heure'");
    ...
    if ($_GET['action'] == 'saveQualification' && isset($_GET['statusGroup'])) {
        ...
        $db->query("DELETE FROM wbcc_status_vocalcom WHERE StatusGroup = $status");
        $db->execute();
        ...
        foreach ($_POST as $key => $stat) {
            ...
            $db->query("INSERT INTO wbcc_status_vocalcom(Oid,StatusGroup,StatusCode,StatusDetail,StatusText,Comment,Positive,Argued,Defaut,Cost,Currency,ValidQuota,Shared) VALUES (:Oid,:StatusGroup,:StatusCode,:StatusDetail,:StatusText,:Comment,:Positive,:Argued,:Defaut,:Cost,:Currency,:ValidQuota,:Shared)");
...
function getQualificationsArgByDID($DID)
{
    ...
    $db = new Database();
    $db->query("SELECT * FROM wbcc_campagne_vocalcom camp,wbcc_status_vocalcom stat WHERE DID ='$DID' AND camp.StatusGroup = stat.StatusGroup AND Argued='1' AND Positive='0'");
    return $db->resultSet();
}
...
function getQualificationsPosByDID($DID)
{
    ...
    $db = new Database();
    $db->query("SELECT * FROM wbcc_campagne_vocalcom camp,wbcc_status_vocalcom stat WHERE DID ='$DID' AND camp.StatusGroup = stat.StatusGroup AND Positive='1' AND Argued='1'");
    return $db->resultSet();
}
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_campagne_vocalcom WHERE Oid = :oid");
    $db->bind("oid", $oid, null);
    return $db->single();
}
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_fa_vocalcom WHERE INDICE = :indice AND DID_CAMPAGNE = :DID");
    $db->bind("DID", $DID, null);
    $db->bind("indice", $indice, null);
    return $db->single();
}
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_agent_vocalcom WHERE OutCampaigns LIKE '%$DID%'");
    $agents = [];
    $data1 = $db->resultSet();
    ...
    foreach ($data1 as $key => $agent) {
        ...
        //Appels émis du jour
        $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa  WHERE DID_CAMPAGNE = '$DID' AND DATE = '$yesterday' AND fa.STATUSGROUP = $StatusGroup AND ID_TV='$login'");
        $appels = $db->single();
        ...
        foreach ($statusArg as $key => $stat) {
            $statusCode = $stat->StatusCode;
            $statusDetail = $stat->StatusDetail;
            $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa WHERE DID_CAMPAGNE = '$DID'  AND fa.STATUS = $statusCode AND fa.DETAIL = $statusDetail AND DATE = '$yesterday' AND ID_TV='$login'");
            $res = $db->single();
            $data[] = [
                "statutArg" => "Nombre Appels " . $stat->StatusText,
                "nbAppels" => my_formatNumberEspace($res->nbAppels)
        ...
        $recordsPlusLongsJ = getRecordsLongsByCampagneAndAgentJ($DID, $agent->LoginName, $yesterday);
        $recordsPlusCourtsJ = getRecordsCourtsByCampagneAndAgentJ($DID, $agent->LoginName, $yesterday);
        $recordsHasardJ = getRecordsHasardByCampagneAndAgentJ($DID, $agent->LoginName, $yesterday);
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_agent_vocalcom WHERE OutCampaigns LIKE '%$DID%' AND LoginName='$login' LIMIT 1");
    $agent = $db->single();
    $today = date("d-m-Y");
    $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $login = $agent->LoginName;
    //Appels émis du jour
    $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa  WHERE DID_CAMPAGNE = '$DID'  AND fa.STATUSGROUP = $StatusGroup AND ID_TV='$login'");
    $appels = $db->single();
    ...
    foreach ($statusArg as $key => $stat) {
        $statusCode = $stat->StatusCode;
        $statusDetail = $stat->StatusDetail;
        $db->query("SELECT COUNT(*) as nbAppels, SUM(DUREE) as duree FROM wbcc_fa_vocalcom as fa WHERE DID_CAMPAGNE = '$DID'  AND fa.STATUS = $statusCode AND fa.DETAIL = $statusDetail AND ID_TV='$login'");
        $res = $db->single();
        $data[] = [
            "statutArg" => "Nombre Appels " . $stat->StatusText,
            "nbAppels" => my_formatNumberEspace($res->nbAppels)
...
function getRecordsLongsByCampagneAndAgentJ($DID, $agent, $date)
{
    ...
    // $today = date("d-m-Y");
    // $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $db->query("SELECT * FROM wbcc_record_vocalcom  WHERE DID='$DID' AND codeAgent='$agent' AND date ='$date' ORDER BY tailleFichier DESC LIMIT 10");
    return $db->resultSet();
}
...
function getRecordsCourtsByCampagneAndAgentJ($DID, $agent, $date)
{
    ...
    // $today = date("d-m-Y");
    // $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $db->query("SELECT * FROM wbcc_record_vocalcom  WHERE DID='$DID' AND codeAgent='$agent' AND date ='$date' ORDER BY tailleFichier ASC LIMIT 10");
    return $db->resultSet();
}
...
function getRecordsHasardByCampagneAndAgentJ($DID, $agent)
{
    ...
    $today = date("d-m-Y");
    $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $db->query("SELECT * FROM wbcc_record_vocalcom  WHERE DID='$DID' AND codeAgent='$agent' AND date ='$yesterday' ORDER BY  RAND() LIMIT 10");
    return $db->resultSet();
}
...
function getRecordsLongsByCampagneAndAgent($DID, $agent)
{
    ...
    $today = date("d-m-Y");
    $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $db->query("SELECT * FROM wbcc_record_vocalcom  WHERE DID='$DID' AND codeAgent='$agent'  ORDER BY tailleFichier DESC LIMIT 10");
    return $db->resultSet();
}
...
function getRecordsCourtsByCampagneAndAgent($DID, $agent)
{
    ...
    $today = date("d-m-Y");
    $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $db->query("SELECT * FROM wbcc_record_vocalcom  WHERE DID='$DID' AND codeAgent='$agent'  ORDER BY tailleFichier ASC LIMIT 10");
    return $db->resultSet();
}
...
function getRecordsHasardByCampagneAndAgent($DID, $agent)
{
    ...
    $today = date("d-m-Y");
    $yesterday =  date("Ymd", strtotime($today . "-1 days"));
    $db->query("SELECT * FROM wbcc_record_vocalcom  WHERE DID='$DID' AND codeAgent='$agent'  ORDER BY RAND() LIMIT 10");
    return $db->resultSet();
}
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_record_vocalcom WHERE DID='$DID'  ORDER BY tailleFichier DESC LIMIT 10");
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_record_vocalcom WHERE DID='$DID' ORDER BY tailleFichier ASC LIMIT 10");
...
{
    $db = new Database();
    $db->query("SELECT * FROM wbcc_record_vocalcom WHERE DID='$DID' ORDER BY RAND() LIMIT 10");
